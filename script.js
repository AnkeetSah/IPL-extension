...
async function fetchAndDisplayMatchData() {
  try {
    const response = await fetch(
      "https://api.cricapi.com/v1/currentMatches?apikey=01156d51-8fb5-4b33-a42e-d785c5646856&offset=0"
    );
    ...
    const matches = data.filter(
      (element) =>
        element.series_id === "76ae85e2-88e5-4e99-83e4-5f352108aebc" &&
    ...
    matches.forEach((match) => {
      ...
      const scoreDetails = score
        .map(
          (inning, index) => `
                        <p><strong>Inning ${index + 1}:</strong> ${
            inning.inning
          } - Runs: ${inning.r}, Wickets: ${inning.w}, Overs: ${inning.o}</p>
                    `
        )
        .join("");
